...
class ActivityExtraction:
    ...
    def from_dict(cls, value: Any) -> ActivityExtraction:
        ...
        if not isinstance(evidence, list) or not evidence:
            raise ExtractionValidationError("visual_evidence must be non-empty")
        if len(evidence) > 5:
            raise ExtractionValidationError("visual_evidence has too many items")
        ...
        return cls(
            ...
            visual_evidence=tuple(dict.fromkeys(evidence)),
        )


def apply_group_policy(extraction: ActivityExtraction) -> dict[str, object]:
    if (
